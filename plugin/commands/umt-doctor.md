...
1. Run `npx universal-mcp-toolkit@1.6.27 doctor` for a full pass over the server registry: missing build outputs, missing environment variables, and config health.
2. If the user names a specific server, scope it: `npx universal-mcp-toolkit@1.6.27 doctor <server-id>`.
3. For each issue reported:
   ...
   - Missing build output (workspace mode) → offer `npx universal-mcp-toolkit@1.6.27 doctor --fix`, which auto-builds the local package.
